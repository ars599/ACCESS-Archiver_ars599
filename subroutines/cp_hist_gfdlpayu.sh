...
# atm
echo -e "\ncopying $( cat $here/tmp/$loc_exp/hist_atm_files.csv | wc -l ) ocean files"
...
# lnd
echo -e "\ncopying $( cat $here/tmp/$loc_exp/hist_lnd_files.csv | wc -l ) ocean files"
curdir=$arch_dir/$loc_exp/history/atm
mkdir -p $curdir; chgrp $arch_grp $curdir
rm -f $curdir/*_tmp* 2>/dev/null
while IFS=, read -r file; do
...
done < $here/tmp/$loc_exp/hist_lnd_files.csv
rm -f $curdir/*_tmp
