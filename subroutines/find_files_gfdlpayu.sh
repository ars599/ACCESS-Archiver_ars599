...
rm -f $here/tmp/$loc_exp/hist_lnd_files.csv
...
for payudir in $payudirs; do
  if [[ ! -f ${payudir}/time_stamp.out ]]; then
    continue
  fi
  echo $payudir
  yarr=()
  ...
  for atmfile in $( ls ${payudir}/atmos_*.nc ); do
    ls $atmfile >> $here/tmp/$loc_exp/hist_atm_files.csv
  done
  for lndfile in $( ls ${payudir}/land_*.nc ); do
    ls $lndfile >> $here/tmp/$loc_exp/hist_lnd_files.csv
  done
  for ocnfile in $( ls ${payudir}/ocean_*.nc* ); do
    ls $ocnfile >> $here/tmp/$loc_exp/hist_ocn_files.csv
  done
  if [ -d ${payudir}/HISTORY/ ]; then 
    for icefile in $( ls ${payudir}/HISTORY/ice*.nc ); do
      ls $icefile >> $here/tmp/$loc_exp/hist_ice_files.csv
    done
  else
    for icefile in $( ls ${payudir}/ice*.nc ); do
      ls $icefile >> $here/tmp/$loc_exp/hist_ice_files.csv
    done
  fi
done
